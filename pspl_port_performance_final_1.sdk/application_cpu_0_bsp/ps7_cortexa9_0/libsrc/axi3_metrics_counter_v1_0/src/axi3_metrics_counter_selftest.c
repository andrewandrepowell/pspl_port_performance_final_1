 ...
 *
 * @param   baseaddr_p is the base address of the AXI3_METRICS_COUNTERinstance to be worked on.
 *
 * @return
 *
 ...
 *
 */
XStatus AXI3_METRICS_COUNTER_Mem_SelfTest(void * baseaddr_p)
{
	int     Index;
	u32 baseaddr;
	u32 Mem32Value;
	...
	{
	  AXI3_METRICS_COUNTER_mWriteMemory(baseaddr+4*Index, (0xDEADBEEF % Index));
	}

	for ( Index = 0; Index < 16; Index++ )
	{
	  Mem32Value = AXI3_METRICS_COUNTER_mReadMemory(baseaddr+4*Index);
