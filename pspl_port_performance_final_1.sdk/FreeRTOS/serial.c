...
xComPortHandle xVirtualPortInitMinimal( unsigned long ulWantedBaud,  UBaseType_t uxQueueLength )
{
	...
	THE TOP OF THIS FILE REGARDING THE QUEUE OF QUEUES FOR THIS PURPSOE. */
	xRxQueue = xQueueCreate( uxQueueLength, sizeof( char ) );
	xTxQueue = xQueueCreate( uxQueueLength, sizeof( char ) );
	configASSERT( xRxQueue );
...
void xVirtualPutCharRx( xComPortHandle pxPort, signed char cOutChar)
{
	...
	/* Send the character. */
	xQueueSend( xRxQueue, &cOutChar, portMAX_DELAY);
}
...
void xVirtualPutStringRx( xComPortHandle pxPort, const char* string)
{
	...
	for (;*string;string++)
	{
		xQueueSend( xRxQueue, string, portMAX_DELAY);
	}
}
...
void vVirtualPutString( xComPortHandle pxPort, const signed char* pcString, unsigned short usStringLength ) {
	...
	signed char* end_ptr = pcString+usStringLength+1;

	for (;pcString!=end_ptr;pcString++) {
		xQueueSend( xTxQueue, pcString, portMAX_DELAY);
	}
}
...
void xVirtualGetCharTx( xComPortHandle pxPort, signed char *TxChar) {
	...
	(void) pxPort;
	xQueueReceive( xTxQueue, TxChar, portMAX_DELAY );
}
...
char cChar;
	...
	if( ( ulActiveInterrupts & serRECEIVE_INTERRUPT_MASK ) != 0 )
	{
		...
		while( ( ulChannelStatusRegister & XUARTPS_SR_RXEMPTY ) == 0 )
		{
			...
			task. */
			configASSERT(xQueueIsQueueFullFromISR(xRxQueue)==pdFALSE); /* Andrew Powell - Added to signal data loss. */
